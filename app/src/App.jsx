...
${q.kind === "gist" ? `GIST = ONE LINE, NOT A SHORTENED PARAGRAPH: the reference must be ONE sentence (one main clause; at most one semicolon or "and" joining a second short clause) capturing problem + core action (+ result/timeline only if central) — everything else gets cut, not lightly reworded. If your first draft reference has more than one period, compress it further before returning it. Score the learner the same way: restating multiple source sentences back-to-back with minor rewording is NOT a passing gist even if factually accurate — dock it as "structure-misread" for failing to compress, and say so in the explanation.
` : ""}
EXPLANATION FAITHFULNESS: when your explanation describes what the learner wrote (e.g. "your version says X"), X must be an exact substring quoted verbatim from Learner's answer above — never paraphrase or invent a misquote of different wording. If you need to characterize their approach without quoting, describe it abstractly instead of putting words in quotes that they did not write.
    ...
    <div style={{ background: C.surface, borderRadius: 16, padding: 22, boxShadow: "0 1px 4px rgba(16,29,51,.06)", borderTop: `5px solid ${LINES[q.line].color}` }}>
      ...
      <div style={{ background: C.bg, borderRadius: 10, padding: 14, marginBottom: 14, display: isBatch ? "none" : "block" }}>
        {q.kind === "gist" && q.scaffold && Array.isArray(q.anatomy) && q.anatomy.length ? (
          <div>
            <p style={{ fontSize: 12, color: C.sub, margin: "0 0 8px" }}>🪜 新手脚手架：功能标签已帮你标好（答对 3 次后撤除）——该留的信息已经挑好，你要练的是把它们压成一句话（合并、删细节、名词化句式改主动语态）</p>
